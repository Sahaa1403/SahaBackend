...
from .models import KnowledgeBase, KnowledgeBaseProcessStatus, Label, KnowledgeBaseLabelUser, User
...
@receiver(post_save, sender=KnowledgeBase)
def create_process_status_for_kb_made(sender, instance, created, **kwargs):
    if not created:
        return  # فقط برای رکوردهای جدید اجرا میشه
    ...
    try:
        ...
        # بررسی اینکه قبلاً این لیبل به این رکورد داده نشده باشه
        exists = KnowledgeBaseProcessStatus.objects.filter(
            knowledge_base=instance,
        ).exists()

        if not exists:
            KnowledgeBaseProcessStatus.objects.create(
                knowledge_base=instance
            )
    except Exception as e:
        print("خطا در سیگنال:", e)